...
class AttnCell(RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with vs.variable_scope(scope or "attention_cell"):
            ...
            self.Alpha_t = masked_softmax(Mt_linear, self.Premise_seqLen)    # shape(b_sz, tstp_pre)

            tmp1 = tf.reduce_sum(tf.expand_dims(self.Alpha_t, 2) * self.Premise_out, axis=1)    # shape(b_sz, h_sz)
